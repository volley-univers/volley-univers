...
const firebaseConfig = {
    apiKey: "AIzaSyCCqdamdGEMBqgKHKaNMBsmNY_-krGcIf4",
...
$('btn-register').addEventListener('click', async () => {
    ...
    try {
        ...
        // 2. Crée le profil utilisateur dans Firestore
...
onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
        // Attend que le token soit vraiment prêt côté Firestore
        let token = null;
        for (let i = 0; i < 5; i++) {
            try {
        ...
        // Récupère le profil depuis Firestore avec retry
        let profileSnap;
        for (let i = 0; i < 3; i++) {
            try {
                ...
                console.warn('Profil non chargé:', e.code);
                if (i < 2) await new Promise(r => setTimeout(r, 1000));
                else profileSnap = { exists: () => false };
            }
        }
...
async function subscribeAnecdotes() {
    ...
    unsubAnecdotes = onSnapshot(q, (snapshot) => {
        ...
        console.warn('Anecdotes snapshot error:', error.code);
        if (error.code === 'permission-denied') {
            setTimeout(() => { if (currentUser) subscribeAnecdotes(); }, 2000);
        }
    });
}
